...
const PLAN_COMPTABLE_STANDARD = [
  ...
  { numero: "1010", nom: "Caisse principale", type: "ACTIF" },
  ...
  { numero: "2110", nom: "Marge de crédit", type: "PASSIF" },
  { numero: "2120", nom: "Carte de crédit Visa", type: "PASSIF" },
  { numero: "2130", nom: "Carte de crédit Mastercard", type: "PASSIF" },
...
async function posterFactureEmise(bon, facture, creePar) {
  ...
  for (const pr of bon.problemes) {
    for (const l of pr.pieces) {
      coutTotal += l.qte * (l.coutant ?? l.piece?.coutant ?? 0);
    }
  }
  if (coutTotal > 0.005) {
...
// Appelé quand une facture passe à "Payée" — enregistre l'encaissement dans
// le compte de trésorerie choisi (compte GL "1000" par défaut si aucun n'est
// précisé, pour les appels existants qui ne passent pas encore ce choix).
async function posterFacturePayee(facture, creePar, compteNumero = "1000", modePaiement = null) {
  if (!(await moduleComptabiliteActif())) return; // module désactivé — aucune écriture créée
  await assurerPlanComptable();
  ...
  const reference = facture.referenceVersement ? ` (réf. ${facture.referenceVersement})` : "";
  await enregistrerEcriture({
    date: facture.datePaiement || new Date(),
    description: `Facture ${facture.numero} payée${suffixe}${reference}`,
    source: "FACTURE_PAYEE",
    sourceId: facture.id,
    lignes: [
      { compteNumero, debit: facture.totalAvecTaxes, description: `Facture ${facture.numero}` },
...
// Compare la valeur du stock du menu Inventaire (Σ qté × coûtant) au solde du
// compte 1200 au grand livre. Une pièce ajoutée à un bon sort du stock tout de
// suite, mais le grand livre n'est crédité qu'à l'émission de la facture : la
// valeur des pièces sur des bons NON facturés explique donc un écart normal
// et temporaire, déduit du solde attendu (jamais aligné de force).
async function calculerAlignementInventaire() {
  const arrondi = (x) => Math.round(x * 100) / 100;
  const [pieces, agregat, utilisees] = await Promise.all([
    prisma.piece.findMany({ select: { qte: true, coutant: true } }),
    prisma.ligneEcriture.aggregate({ where: { compte: { numero: "1200" } }, _sum: { debit: true, credit: true } }),
    prisma.pieceUtilisee.findMany({
      where: { probleme: { bon: { facture: null } } },
      select: { qte: true, coutant: true, piece: { select: { coutant: true } } },
    }),
  ]);
  const valeurStock = arrondi(pieces.reduce((s, p) => s + p.qte * p.coutant, 0));
  const soldeGL = arrondi((agregat._sum.debit || 0) - (agregat._sum.credit || 0));
  const surBonsNonFactures = arrondi(utilisees.reduce((s, u) => s + u.qte * (u.coutant ?? u.piece?.coutant ?? 0), 0));
  const ecart = arrondi(valeurStock - (soldeGL - surBonsNonFactures));
  return { valeurStock, soldeGL, surBonsNonFactures, ecart };
}

// Ramène le compte 1200 à la valeur réelle du stock par UNE écriture dont la
// contrepartie est toujours "Soldes d'ouverture" (3010) — jamais un compte de
// résultat, donc aucun effet sur le bénéfice. Comme l'écart est calculé à
// partir du solde réel du 1200, un solde d'ouverture déjà saisi (ou des
// achats déjà reçus par facture fournisseur) est pris en compte : on ne
// comptabilise jamais deux fois la même valeur. Retourne null si rien à faire.
async function alignerInventaireAuGL(creePar, motif) {
  if (!(await moduleComptabiliteActif())) return null;
  await assurerPlanComptable();

  const a = await calculerAlignementInventaire();
  if (Math.abs(a.ecart) < 0.005) return null;

  const hausse = a.ecart > 0;
  const montant = Math.abs(a.ecart);
  const description = `${motif} — alignement inventaire / grand livre (stock ${a.valeurStock.toFixed(2)} $, G/L ${a.soldeGL.toFixed(2)} $)`;
  return enregistrerEcriture({
    date: new Date(),
    description,
    source: "ALIGNEMENT_INVENTAIRE",
    lignes: [
      { compteNumero: "1200", debit: hausse ? montant : 0, credit: hausse ? 0 : montant, description },
      { compteNumero: "3010", debit: hausse ? 0 : montant, credit: hausse ? montant : 0, description: "Contrepartie inventaire" },
    ],
    creePar,
  });
}
...
// Plage de départ par type de compte, utilisée quand aucun compte de ce
// type n'existe encore (borne basse de la convention 1xxx=Actif, 2xxx=Passif,
// 3xxx=Capitaux propres, 4xxx=Revenus, 5xxx=Dépenses)
const DEPART_NUMERO_PAR_TYPE = { ACTIF: 1000, PASSIF: 2000, CAPITAUX_PROPRES: 3000, REVENU: 4010, DEPENSE: 5100 };

// Suggère le prochain numéro disponible pour un type de compte donné, sans
// créer le compte (contrairement à creerCompteAvecProchainNumero) — utilisé
// pour pré-remplir le formulaire "+ Compte" du plan comptable, que
// l'utilisateur reste libre de modifier avant de sauvegarder.
async function prochainNumeroCompte(type) {
  const depart = DEPART_NUMERO_PAR_TYPE[type] ?? 1000;
  const comptes = await prisma.compte.findMany({ where: { type } });
  const max = comptes.reduce((m, c) => Math.max(m, parseInt(c.numero, 10) || 0), depart);
  return String(max + 10);
}
...
// Poste système utilisé uniquement quand une ligne de facture fournisseur
// est liée à une réception d'inventaire (voir app/api/depenses/route.js) —
// débite l'actif "Inventaire de pièces" (1200) plutôt qu'une dépense, pour
// que l'achat augmente la valeur de l'inventaire au bilan au lieu de passer
// directement en charge. Le coût ne devient une dépense (5000) qu'au moment
// de la vente, via l'écriture de coût des marchandises vendues déjà en place
// dans posterFactureEmise. Non offert dans la liste normale des postes de
// dépenses (voir CODE_CATEGORIE_INVENTAIRE) — assigné automatiquement par
// l'interface, jamais choisi manuellement.
const CODE_CATEGORIE_INVENTAIRE = "INVENTAIRE_PIECES";
async function assurerCategorieInventairePieces() {
  return prisma.categorieDepense.upsert({
    where: { code: CODE_CATEGORIE_INVENTAIRE },
    update: {},
    create: { code: CODE_CATEGORIE_INVENTAIRE, nom: "Réception d'inventaire (actif)", compteDepenseNumero: "1200" },
  });
}
...
// `depense.lignesPourEcriture` : une entrée par poste de dépense (une même
// facture peut être fractionnée sur plusieurs comptes, ex. pneus + papeterie).
async function posterDepenseRecue(depense, creePar) {
  ...
  const lignes = depense.lignesPourEcriture.map((l) => ({
    compteNumero: l.compteDepenseNumero,
    debit: l.montant,
    description: l.description || depense.description,
  }));
...
// Appelé quand une dépense est marquée payée — sort l'argent du compte de
// trésorerie choisi (compte GL "1000" par défaut si aucun n'est précisé, pour
// les appels existants qui ne passent pas encore ce choix).
async function posterDepensePayee(depense, creePar, compteNumero = "1000", modePaiement = null) {
  if (!(await moduleComptabiliteActif())) return;
  await assurerPlanComptable();
  const { LABEL_MODE_PAIEMENT } = require("./modesPaiement");
  const suffixe = modePaiement && LABEL_MODE_PAIEMENT[modePaiement] ? ` — ${LABEL_MODE_PAIEMENT[modePaiement]}` : "";
  const description = `Paiement — ${depense.description}${depense.referenceVersement ? ` (réf. ${depense.referenceVersement})` : ""}${suffixe}`;
  await enregistrerEcriture({
    ...
    lignes: [
      ...
      { compteNumero, credit: depense.montant, description },
    ],
    creePar,
  });
}
...
module.exports = { assurerPlanComptable, assurerCategoriesInventaire, assurerCategoriesDepense, assurerCategorieInventairePieces, CODE_CATEGORIE_INVENTAIRE, prochainNumeroDepense, prochainNumeroRevenu, prochainNumeroCompte, creerCompteAvecProchainNumero, obtenirStatutPeriode, verifierPeriodeModifiable, enregistrerEcriture, renverserEcriture, posterFactureEmise, posterFacturePayee, posterPaie, calculerAlignementInventaire, alignerInventaireAuGL, posterDepenseRecue, posterDepensePayee, posterAcquisitionImmobilisation, posterAmortissementMensuel, posterRemiseGouvernementale, enregistrerSoldesOuverture, PLAN_COMPTABLE_STANDARD, COMPTE_MAIN_OEUVRE, COMPTES_REVENU_RESERVES };
