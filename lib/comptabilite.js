...
// Retourne la date de verrouillage actuelle (ou null si aucune) — tout ce
// qui est daté à cette date ou avant est protégé contre la modification
// et la suppression, pour préserver un mois déjà clôturé
async function dateVerrouillage() {
  const p = await prisma.parametre.findUnique({ where: { cle: "comptabilite_verrouille_avant" } });
  return p?.valeur ? new Date(p.valeur) : null;
}

// Lance une erreur si la date donnée tombe dans une période verrouillée
async function verifierNonVerrouille(date) {
  const verrou = await dateVerrouillage();
  if (verrou && new Date(date) <= verrou) {
    const dateTexte = verrou.toLocaleDateString("fr-CA", { timeZone: "America/Toronto" });
    throw new Error(`VERROUILLE:Cette période est verrouillée (jusqu'au ${dateTexte}) — déverrouille-la d'abord dans Comptabilité si c'est vraiment nécessaire.`);
  }
}
...
const PLAN_COMPTABLE_STANDARD = [
  ...
  { numero: "5000", nom: "Coût des pièces vendues", type: "DEPENSE" },
  { numero: "5100", nom: "Dépenses générales", type: "DEPENSE" },
  { numero: "5200", nom: "Salaires", type: "DEPENSE" },
  { numero: "5210", nom: "Charges sociales de l'employeur", type: "DEPENSE" },
  { numero: "5215", nom: "Vacances", type: "DEPENSE" },
  { numero: "5220", nom: "Électricité", type: "DEPENSE" },
  { numero: "5230", nom: "Loyer", type: "DEPENSE" },
  { numero: "5240", nom: "Assurances", type: "DEPENSE" },
  { numero: "5250", nom: "Téléphone et Internet", type: "DEPENSE" },
  { numero: "5260", nom: "Entretien et réparations", type: "DEPENSE" },
  { numero: "5270", nom: "Fournitures de bureau", type: "DEPENSE" },
  { numero: "5280", nom: "Publicité et marketing", type: "DEPENSE" },
  { numero: "5290", nom: "Frais bancaires et professionnels", type: "DEPENSE" },
  { numero: "5300", nom: "Amortissement", type: "DEPENSE" },
];

// Crée les comptes manquants du plan standard (idempotent — ne duplique
// jamais un compte déjà existant, identifié par son numéro)
async function assurerPlanComptable() {
  for (const c of PLAN_COMPTABLE_STANDARD) {
    await prisma.compte.upsert({
      where: { numero: c.numero },
      update: { nom: c.nom, type: c.type },
      create: c,
    });
  }
}
...
async function enregistrerEcriture({ date, description, source, sourceId, lignes, creePar }) {
...
// Appelé quand une facture officielle est émise — enregistre la vente
// (compte client débité, revenus et taxes à payer crédités)
const COMPTE_PAR_CATEGORIE_TACHE = {
  MAIN_OEUVRE: "4000", ALIGNEMENT: "4030", REMORQUAGE: "4050", ENTREPOSAGE: "4040", AUTRE: "4060",
};
...
async function posterFactureEmise(bon, facture, creePar) {
  ...
  // Main-d'œuvre par catégorie de tâche
  const mainOeuvreParCategorie = {};
  for (const pr of bon.problemes) {
    const heures = pr.entreesTemps.filter((t) => t.fin).reduce((s, t) => s + (new Date(t.fin) - new Date(t.debut)) / 3600000, 0);
    const cat = pr.categorieRevenu || "MAIN_OEUVRE";
    mainOeuvreParCategorie[cat] = (mainOeuvreParCategorie[cat] || 0) + heures * facture.tauxHoraireUtilise;
  }
  ...
  for (const [cat, montant] of Object.entries(mainOeuvreParCategorie)) {
    const apresEscompte = montant * (1 - ratioEscompte);
    if (apresEscompte > 0.005) lignes.push({ compteNumero: COMPTE_PAR_CATEGORIE_TACHE[cat] || "4000", credit: apresEscompte, description: `Facture ${facture.numero}` });
  }
