...
// Propre à VR Premium — le modèle Vehicule n'existe pas dans le template
// général (voir prisma/schema.prisma). Recherche par marque, modèle, plaque,
// NIV ou nom du propriétaire.
async function rechercherVehicule(session, { marque, modele, plaque, vin, clientNom } = {}) {
  if (!(await aAccesSection(session, "clients"))) {
    return { erreur: "Accès refusé à la section Clients pour cet employé." };
  }
  if (!marque && !modele && !plaque && !vin && !clientNom) {
    return { erreur: "Précise au moins un critère : marque, modèle, plaque, NIV ou nom de client." };
  }

  const where = {};
  if (marque) where.marque = { contains: marque, mode: "insensitive" };
  if (modele) where.modele = { contains: modele, mode: "insensitive" };
  if (plaque) where.plaque = { contains: plaque, mode: "insensitive" };
...
async function rechercherDepenses(session, { categorie, fournisseurNom, mois, annee } = {}) {
  ...
  const where = {};
  if (categorie) where.categorieDepense = { nom: { contains: categorie, mode: "insensitive" } };
  ...
  const [agrege, depenses] = await Promise.all([
    ...
    prisma.depense.findMany({ where, include: { fournisseur: true, categorieDepense: true }, orderBy: { dateFacture: "desc" }, take: 25 }),
  ]);
  ...
  return {
    ...
    depenses: depenses.map((d) => ({
      ...
      categorie: d.categorieDepense.nom,
...
const OUTILS = [
  ...
  },
  {
    ...
    declaration: {
      ...
      description: "Recherche un véhicule par marque, modèle, plaque, NIV et/ou nom du client propriétaire, et retourne le propriétaire.",
      parameters: {
        type: "OBJECT",
        properties: {
          marque: { type: "STRING", description: "Marque du véhicule (ex: Honda, Toyota)" },
          modele: { type: "STRING", description: "Modèle du véhicule (ex: Civic, Corolla)" },
          plaque: { type: "STRING", description: "Plaque d'immatriculation (ou partie)" },
          vin: { type: "STRING", description: "NIV / numéro de série (ou partie)" },
          clientNom: { type: "STRING", description: "Nom (ou partie du nom) du client propriétaire" },
        },
      },
    },
  },
