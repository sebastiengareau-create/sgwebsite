...
const { joursCalendairesQuebec, dateQuebecStr } = require("./temps");

const CLES_HORAIRE = ["heures_lun", "heures_mar", "heures_mer", "heures_jeu", "heures_ven", "heures_sam", "heures_dim"];
const DEFAUTS_HORAIRE = { lun: 8, mar: 8, mer: 8, jeu: 8, ven: 5, sam: 0, dim: 0 };
 ...
 * `salaireAnnuelEstime` = base annuelle à utiliser pour déterminer le palier
 * d'impôt (RRQ/RQAP/AE restent calculés sur le vrai brut de cette paie-ci).
 * Si omis, on retombe sur l'ancienne méthode (brut de cette paie × nombre de
 * périodes/an) — utile pour une paie de vacances, qui n'a pas d'heures
 * "habituelles".
 */
function calculerPaie(salaireBrutPeriode, frequencePaie, dejaVerseAnnee = { rrq: 0, rqap: 0, ae: 0 }, salaireAnnuelEstime = null) {
  const periodesParAn = PERIODES_PAR_AN[frequencePaie] || 26;
  const brutAnnuelPourImpot = salaireAnnuelEstime != null ? salaireAnnuelEstime : salaireBrutPeriode * periodesParAn;
  ...
  // --- RRQ/RQAP annuels estimés sur la même base que l'impôt (pas les
  // montants réels ci-dessus, qui respectent les maximums déjà versés) —
  // seulement pour arriver au revenu imposable annualisé ---
  const rrqAnnuelPourImpot = Math.min(Math.max(0, brutAnnuelPourImpot - RRQ.exemptionGenerale) * RRQ.tauxBase, RRQ.maxEmploye);
  const rqapAnnuelPourImpot = Math.min(brutAnnuelPourImpot * RQAP.tauxEmploye, RQAP.maxEmploye);

  // --- Revenu imposable annualisé (RRQ et RQAP sont déductibles) ---
  const revenuImposableAnnuel = Math.max(0, brutAnnuelPourImpot - rrqAnnuelPourImpot - rqapAnnuelPourImpot);
...
// Salaire imposable par défaut pour un employé horaire = taux horaire ×
// heures d'ouverture du commerce (l'horaire complet d'une semaine type,
// pas les heures réellement poinçonnées) × 52 semaines. Sert de valeur de
// départ sur la fiche employé (modifiable ensuite) et de base pour
// l'impôt tant que le gérant n'a rien ajusté manuellement.
function calculerSalaireImposableParDefaut(tauxHoraire, horaireOuverture) {
  const heuresParSemaine = Object.values(horaireOuverture).reduce((s, h) => s + h, 0);
  return arrondir(heuresParSemaine * 52 * tauxHoraire);
}

async function estimerSalaireImposableParDefaut(tauxHoraire) {
  const parametresHoraire = await prisma.parametre.findMany({ where: { cle: { in: CLES_HORAIRE } } });
  const dictHoraire = Object.fromEntries(parametresHoraire.map((p) => [p.cle.replace("heures_", ""), Number(p.valeur)]));
  const horaireFinal = { ...DEFAUTS_HORAIRE, ...dictHoraire };
  return calculerSalaireImposableParDefaut(tauxHoraire, horaireFinal);
}
...
async function calculerPaiePourEmploye(employeId, { periodeDebut, periodeFin, typePaie, heuresManuelles, montantVacances, boni } = {}) {
  ...
  // été versé via une paie de vacances précédente. `paiesAnnee` sert aussi
  // plus bas aux maximums annuels (RRQ/RQAP/AE).
  const [accumule, dejaVerse, paiesAnnee] = await Promise.all([
    ...
    prisma.paie.findMany({
      ...
      select: { rrqEmploye: true, rqapEmploye: true, aeEmploye: true },
    }),
  ]);
  ...
  // Base annuelle pour déterminer le palier d'impôt — reste null pour une
  // paie de vacances (voir calculerPaie)
  let salaireAnnuelEstime = null;
  ...
  if (typePaie === "VACANCES") {
    ...
    salaireAnnuelEstime = employe.salaireAnnuel || 0;
  } else {
    const [entrees, entreesInternes, parametreTaux, parametresHoraire] = await Promise.all([
      ...
      prisma.parametre.findMany({ where: { cle: { in: CLES_HORAIRE } } }),
    ]);
    heuresHorodateur =
      entrees.reduce((s, e) => s + dureeHeures(e.debut, e.fin), 0) +
      entreesInternes.reduce((s, e) => s + dureeHeures(e.debut, e.fin), 0);

    // Heures par défaut = l'horaire d'ouverture du commerce (Paramètres),
    // pas les heures réellement poinçonnées :
    // - Secrétaire : toutes les heures d'ouverture prévues sur la période
    //   (peu importe le poinçon — ce rôle ne poinçonne pas par tâche).
    // - Autres (mécanicien…) : seulement les jours où au moins un poinçon
    //   existe, chacun compté pour l'horaire prévu CE jour-là (ex. vendredi
    //   = 5h prévues) plutôt que la durée exacte poinçonnée. Le vrai total
    //   poinçonné (heuresHorodateur) reste affiché à titre indicatif.
    const dictHoraire = Object.fromEntries(parametresHoraire.map((p) => [p.cle.replace("heures_", ""), Number(p.valeur)]));
    const horaireFinal = { ...DEFAUTS_HORAIRE, ...dictHoraire };
    const joursCalendaires = joursCalendairesQuebec(debut, fin);

    let heuresParDefaut;
    if (employe.role === "SECRETAIRE") {
      heuresParDefaut = joursCalendaires.reduce((s, j) => s + (horaireFinal[j.jour] || 0), 0);
    } else {
      const joursTravailles = new Set([...entrees, ...entreesInternes].map((e) => dateQuebecStr(e.debut)));
      heuresParDefaut = joursCalendaires.reduce((s, j) => s + (joursTravailles.has(j.dateStr) ? (horaireFinal[j.jour] || 0) : 0), 0);
    }
    ...
    // Base d'impôt = le "Salaire imposable" de la fiche employé — rempli
    // par défaut (taux horaire × heures d'ouverture du commerce × 52
    // semaines, voir calculerSalaireImposableParDefaut) mais modifiable
    // par le gérant (ex : employé à temps partiel).
    salaireAnnuelEstime = employe.salaireImposable != null
      ? employe.salaireImposable
      : calculerSalaireImposableParDefaut(taux, horaireFinal);
  }

  // Le boni s'ajoute au brut de base et suit les mêmes déductions — jamais
  // appliqué sur une paie de vacances (le montant de vacances EST déjà le
  // versement complet).
  const boniFinal = typePaie === "VACANCES" ? 0 : Number(boni) || 0;
  const salaireBase = salaireBrutPeriode;
  salaireBrutPeriode += boniFinal;
  if (salaireAnnuelEstime != null) salaireAnnuelEstime += boniFinal * periodesParAn;
  ...
  const resultat = calculerPaie(salaireBrutPeriode, employe.frequencePaie, dejaVerseAnnee, salaireAnnuelEstime);
  const vacancesAccumulees = typePaie === "VACANCES" ? 0 : Math.round(salaireBrutPeriode * (employe.tauxVacances / 100) * 100) / 100;

  return {
    employe: {
      id: employe.id, nom: employe.nom, typeRemuneration: employe.typeRemuneration, frequencePaie: employe.frequencePaie,
      tauxVacances: employe.tauxVacances, tauxHoraireEmploye: employe.tauxHoraireEmploye, salaireAnnuel: employe.salaireAnnuel,
    },
    heuresTravaillees,
    heuresHorodateur,
    salaireBase,
...
module.exports = {
  calculerPaie, calculerPaiePourEmploye, estimerSalaireImposableParDefaut,
  PERIODES_PAR_AN, RRQ, RQAP, AE,
};
