...
// Clé courte ("lun".."dim", même convention que Parametre "heures_XXX") du
// jour de semaine d'une date, en heure du Québec.
function jourSemaineQuebec(date) {
  const jourAnglais = new Intl.DateTimeFormat("en-US", { timeZone: FUSEAU, weekday: "short" }).format(new Date(date));
  return { Sun: "dim", Mon: "lun", Tue: "mar", Wed: "mer", Thu: "jeu", Fri: "ven", Sat: "sam" }[jourAnglais];
}

// "YYYY-MM-DD" du jour calendaire réel d'une date en heure du Québec, peu
// importe l'heure UTC de stockage — utilisé pour regrouper des entrées de
// temps par jour travaillé.
function dateQuebecStr(date) {
  return new Intl.DateTimeFormat("en-CA", { timeZone: FUSEAU, year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(date));
}

// Liste chaque jour calendaire (Québec) entre deux instants inclusivement,
// avec son jour de semaine — utilisé pour calculer les heures d'ouverture
// attendues sur une période de paie (voir lib/paie.js).
function joursCalendairesQuebec(debut, fin) {
  const jours = [];
  let curseurStr = dateQuebecStr(debut);
  const finStr = dateQuebecStr(fin);
  let curseur = new Date(`${curseurStr}T12:00:00Z`); // midi UTC : jamais près d'un changement d'heure d'été/hiver
  while (curseurStr <= finStr) {
    jours.push({ dateStr: curseurStr, jour: jourSemaineQuebec(curseur) });
    curseur = new Date(curseur.getTime() + 86400000);
    curseurStr = dateQuebecStr(curseur);
  }
  return jours;
}

module.exports = { dateAujourdhuiQuebec, limitesJourQuebec, limitesMoisQuebec, dateHeureQuebecVersUTC, dateHeureLocaleVersUTC, jourSemaineQuebec, dateQuebecStr, joursCalendairesQuebec, FUSEAU };
