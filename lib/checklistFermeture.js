...
async function obtenirChecklist(annee, mois) {
  ...
  const [ecritures, paiesBrouillon, immobilisationsActives, amortissementMois, facturesImpayees, tpsTvq, comptesTresorerie, clientsARecevoir, fournisseursAPayer] = await Promise.all([
    ...
    prisma.compteTresorerie.findMany({
      where: { actif: true },
      orderBy: { ordre: "asc" },
      select: { nom: true, rapprochements: { orderBy: { dateRapprochement: "desc" }, take: 1, select: { dateRapprochement: true } } },
    }),
  ...
  const informatifs = [
    ...
    {
      cle: "conciliation_bancaire",
      label: "Dernière conciliation bancaire",
      // Par compte : concilier la caisse ne dit rien sur la banque.
      valeur: comptesTresorerie.map((c) => {
        const dernier = c.rapprochements[0];
        return `${c.nom} : ${dernier ? new Date(dernier.dateRapprochement).toLocaleDateString("fr-CA", { timeZone: "America/Toronto" }) : "Aucune"}`;
      }).join(" · ") || "Aucune",
    },
