...
// Tous les modèles du schéma, triés parent avant enfant (un modèle vient
// après chaque modèle vers lequel il a une clé étrangère). Dérivé du schéma
// plutôt que tenu à la main : une liste manuelle avait oublié des tables,
// ce qui faisait échouer la réinitialisation et amputait les sauvegardes.
// L'import écrit dans cet ordre ; l'effacement le parcourt à l'envers.
function modelesEnOrdreDeDependance() {
  const modeles = Prisma.dmmf.datamodel.models;
  const parents = new Map(
    modeles.map((m) => [
      m.name,
      m.fields
        .filter((c) => c.kind === "object" && c.relationFromFields?.length > 0 && c.type !== m.name)
        .map((c) => c.type),
    ])
  );
  const ordre = [];
  const etat = new Map(); // 1 = en cours, 2 = placé
  function placer(nom) {
    if (etat.get(nom) === 2) return;
    if (etat.get(nom) === 1) throw new Error(`Cycle de clés étrangères autour du modèle ${nom}`);
    etat.set(nom, 1);
    for (const parent of parents.get(nom)) placer(parent);
    etat.set(nom, 2);
    ordre.push(nom);
  }
  for (const m of modeles) placer(m.name);
  return ordre.map((nom) => nom.charAt(0).toLowerCase() + nom.slice(1));
}
const MODELES = modelesEnOrdreDeDependance();
