...
async function calculerPaiePourEmploye(employeId, { periodeDebut, periodeFin, typePaie, heuresManuelles, montantVacances, boni } = {}) {
  ...
  if (typePaie === "VACANCES") {
    ...
    if (employe.role === "SECRETAIRE") {
      heuresParDefaut = joursCalendairesQuebec(debut, fin).reduce((s, j) => s + (horaireFinal[j.jour] || 0), 0);
    } else {
      heuresParDefaut = heuresPrevuesJoursPoinconnes([...entrees, ...entreesInternes], debut, fin, horaireFinal);
    }
