  ...
  return new Promise((resolve, reject) => {
    ...
    bon.problemes.forEach((pr, idx) => {
      ...
      if (montantManuel > 0.005) {
        doc.fontSize(9).font("Helvetica").fillColor(GRIS)
          .text(`${pr.factureQte || 1} × ${(pr.facturePrixUnitaire || 0).toFixed(2)} $ = ${fmt(montantManuel)}`, 400, doc.y - 13, { width: 162, align: "right" });
      }
      if (!estMainOeuvre && pr.factureDescription) {
        doc.fontSize(9).font("Helvetica").fillColor(GRIS_CLAIR).text(pr.factureDescription, 50, doc.y);
      }
      doc.moveDown(0.15);
