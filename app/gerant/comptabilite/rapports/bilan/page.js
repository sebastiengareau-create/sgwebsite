...
export default async function BilanImprimable() {
  ...
  const actifs = lignes.filter((l) => l.type === "ACTIF" && l.solde !== 0);
  const passifs = lignes.filter((l) => l.type === "PASSIF" && l.solde !== 0);
  const capitauxComptes = lignes.filter((l) => l.type === "CAPITAUX_PROPRES");
  ...
  const equilibre = Math.abs(totalActif - (totalPassif + totalCapitauxPropres)) < 0.01;
    ...
    <div>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto 40px", background: "white", color: "#17150f", padding: "36px 40px", fontFamily: "Arial, sans-serif" }}>
        <div style={{ borderBottom: "2px solid #17150f", paddingBottom: 16, marginBottom: 24 }}>
          ...
          <p style={{ fontSize: 16, fontWeight: 700, marginTop: 12, marginBottom: 0 }}>Bilan</p>
          <p style={{ fontSize: 11.5, color: "#666", margin: "2px 0 0" }}>
            Au {new Date().toLocaleDateString("fr-CA", { timeZone: "America/Toronto" })}
          </p>
        </div>

        <SectionTable titre="Actif" lignes={actifs} total={totalActif} />
        <SectionTable titre="Passif" lignes={passifs} total={totalPassif} />
        <SectionTable
          titre="Capitaux propres"
          lignes={[...capitauxComptes.filter((l) => l.solde !== 0), { numero: "", nom: "Résultat net cumulé (non affecté)", solde: resultatNetCumule }]}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 24, paddingTop: 16, borderTop: "2px solid #17150f" }}>
          <span style={{ fontSize: 14, fontWeight: 700 }}>{equilibre ? "✓ Le bilan s'équilibre" : "⚠ Écart détecté"}</span>
          <span style={{ fontSize: 14, color: "#666" }}>Actif {totalActif.toFixed(2)} $ = Passif + Capitaux {(totalPassif + totalCapitauxPropres).toFixed(2)} $</span>
        </div>
      </div>
    </div>
  );
