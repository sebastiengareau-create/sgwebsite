...
// Vrai seulement pour l'accès développeur (jamais un compte employé) — réservé
// aux outils de mise en place comme les imports de données.
function estDeveloppeur(session) {
  return session?.role === "DEVELOPPEUR";
}
...
const TOUTES_SECTIONS = ["operations", "calendrier", "clients", "fournisseurs", "inventaire", "comptabilite", "paie", "employes", "vue-ensemble", "jobs-temps-reel", "horodateur", "parametres"];
const DEFAUTS_INITIAUX_ROLE = {
  GERANT: TOUTES_SECTIONS,
  SECRETAIRE: ["operations", "calendrier", "clients", "inventaire"],
  // MECANICIEN avait toujours accès à l'horodateur sans exception — reste
  // vrai par défaut ici, mais devient décochable comme le reste.
  MECANICIEN: ["horodateur"],
};
