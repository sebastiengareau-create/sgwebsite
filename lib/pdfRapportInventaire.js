...
// Colonnes : numéro, description, qté, coûtant, vendant, marge
const COLONNES = [
  { cle: "numero", label: "No pièce", largeur: 85, align: "left" },
  { cle: "nom", label: "Description", largeur: 175, align: "left" },
  { cle: "qte", label: "Qté", largeur: 45, align: "right" },
  { cle: "coutant", label: "Coûtant", largeur: 65, align: "right" },
  { cle: "prix", label: "Vendant", largeur: 65, align: "right" },
  { cle: "marge", label: "Marge", largeur: 67, align: "right" },
];
...
function genererPdfRapportInventaire(pieces, nomEntreprise) {
  return new Promise((resolve, reject) => {
    ...
    for (const p of pieces) {
      ...
      const yLigne = doc.y;
      const valeurs = { numero: p.numero, nom: p.nom, qte: String(p.qte), coutant: fmt(p.coutant), prix: fmt(p.prix), marge: fmt(marge) };
      COLONNES.forEach((col, i) => {
        doc.fillColor(col.cle === "marge" && marge < 0 ? "#a83232" : NOIR)
