...
async function inventairePiece(session, { nom } = {}) {
  ...
  const pieces = await prisma.piece.findMany({
    where: filtreMots(nom, ["nom", "numero", "emplacement"].map(contient)),
    orderBy: { nom: "asc" },
    take: 15,
  });
...
async function rendezVousDuJour(session, { date, dateFin, clientNom } = {}) {
  ...
  const dateStr = format.test(date || "") ? date : dateAujourdhuiQuebec();
  const finStr = format.test(dateFin || "") && dateFin >= dateStr ? dateFin : dateStr;
  ...
  },
  {
    ...
    declaration: {
      ...
      parameters: {
        type: "OBJECT",
        properties: {
          date: { type: "STRING", description: "Date (ou début de période) au format AAAA-MM-JJ — vide pour aujourd'hui" },
