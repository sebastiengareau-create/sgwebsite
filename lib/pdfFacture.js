...
function genererPdfFacture(bon, infosEntreprise, dict, tpsTaux, tvqTaux) {
  return new Promise((resolve, reject) => {
    ...
    function ligneTravail(description, { hrs, qte, prix, total }) {
      const y = doc.y;
      doc.fontSize(9).font("Helvetica").fillColor(GRIS).text(description, X_DESC, y, { width: L_DESC });
      if (hrs) doc.fontSize(9).font("Helvetica").fillColor(GRIS).text(hrs, X_HRS, y, { width: L_HRS, align: "right" });
      if (qte) doc.fontSize(9).font("Helvetica").fillColor(GRIS).text(String(qte), X_QTE, y, { width: L_QTE, align: "right" });
      doc.fontSize(9).font("Helvetica").fillColor(GRIS).text(fmt(prix), X_PRIX, y, { width: L_PRIX, align: "right" });
      doc.fontSize(9).font("Helvetica-Bold").fillColor(NOIR).text(fmt(total), X_TOTAL, y, { width: L_TOTAL, align: "right" });
      doc.y = y;
      doc.moveDown(0.35);
    }
