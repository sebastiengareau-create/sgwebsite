...
async function calculerPaiePourEmploye(employeId, { periodeDebut, periodeFin, typePaie, heuresManuelles, montantVacances, boni } = {}) {
  ...
  // plus bas aux maximums annuels (RRQ/RQAP/AE) et à la moyenne d'heures
  // d'un employé horaire (pour l'estimation de l'impôt).
  const [accumule, dejaVerse, paiesAnnee] = await Promise.all([
    ...
    prisma.paie.findMany({
      ...
      select: { typePaie: true, heuresTravaillees: true, rrqEmploye: true, rqapEmploye: true, aeEmploye: true },
    }),
  ]);
  ...
  if (typePaie === "VACANCES") {
    ...
    // Impôt basé sur la MOYENNE des heures réelles de cette année (paies
    // régulières déjà versées + celle-ci), pas sur les heures de cette
    // paie seule ni sur un calcul d'horaire par défaut — ce dernier peut
    // tomber près de 0 si peu de jours sont reconnus comme travaillés
    // cette période précise (ex: horaire irrégulier, fin de semaine hors
    // horaire d'ouverture), ce qui annulerait l'impôt même pour une grosse
    // paie bien réelle. La moyenne amortit quand même les semaines
    // exceptionnelles (trop ou pas assez d'heures) sur l'ensemble de
    // l'année plutôt que de taxer chaque paie isolément.
    const heuresRegulieresAnnee = paiesAnnee.filter((p) => p.typePaie === "REGULIERE").map((p) => p.heuresTravaillees);
    const heuresMoyennes = [...heuresRegulieresAnnee, heuresTravaillees].reduce((s, h) => s + h, 0) / (heuresRegulieresAnnee.length + 1);
    salaireAnnuelEstime = heuresMoyennes * taux * periodesParAn;
  }
