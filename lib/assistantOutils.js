...
const { libelleVehicule, nettoyerNiv } = require("./vehicules");
const { decoderNiv, marquesPour, modelesPour, versionsDuModele, cleMarque, nomMarque } = require("./catalogueVehicules");
...
// ---------- Véhicules ----------
// Conditions sur un véhicule du dossier pour un mot : marque, modèle,
// version, NIV, plaque (avec ou sans espace : « ABC123 » trouve « ABC 123 »)
// ou année (« 2019 »).
function conditionsVehicule(mot) {
  const conditions = ["marque", "modele", "version"].map((champ) => ({ [champ]: { contains: mot, mode: "insensitive" } }));
  const compact = mot.replace(/[\s-]/g, "");
  if (compact.length >= 3) conditions.push({ niv: { contains: nettoyerNiv(compact), mode: "insensitive" } });
  const variantesPlaque = new Set([mot]);
  if (/^[a-z0-9]{4,8}$/i.test(compact)) {
    for (let i = 1; i < compact.length; i++) variantesPlaque.add(`${compact.slice(0, i)} ${compact.slice(i)}`);
  }
  for (const v of variantesPlaque) conditions.push({ plaque: { contains: v, mode: "insensitive" } });
  if (/^(19|20)\d{2}$/.test(mot)) conditions.push({ annee: Number(mot) });
  return conditions;
}
const surVehicule = (mot) => ({ vehicule: { is: { OR: conditionsVehicule(mot) } } }); // bon de travail
const surVehicules = (mot) => ({ vehicules: { some: { OR: conditionsVehicule(mot) } } }); // client

// Véhicule tel que retourné à l'assistant
function infoVehicule(v) {
  if (!v) return null;
  return {
    description: libelleVehicule(v) || null,
    annee: v.annee || null,
    marque: v.marque || null,
    modele: v.modele || null,
    version: v.version || null,
    plaque: v.plaque || null,
    niv: v.niv || null,
  };
}
...
async function rechercherClient(session, { nom } = {}) {
  ...
  const clients = await prisma.client.findMany({
    where: filtreMots(nom, [...["nom", "telephone", "courriel", "numero", "ville", "adresse", "garantieProlongee"].map(contient), surVehicules]),
    include: { _count: { select: { bons: true } }, vehicules: { orderBy: { creeLe: "desc" } } },
    orderBy: { nom: "asc" },
    take: 15,
  });
  ...
  return {
    clients: clients.map((c) => ({
      ...
      garantieProlongee: c.garantieProlongee || null,
      vehicules: c.vehicules.map(infoVehicule),
    })),
  };
}
...
async function rechercherFacture(session, { numero, clientNom, statut } = {}) {
  ...
  return {
    factures: factures.map((f) => ({
      ...
      bon: f.bon.numero,
      vehicule: infoVehicule(f.bon.vehicule),
      lien: lienBon(f.bon),
...
async function rechercherTachesBons(session, { texte } = {}) {
  ...
  return {
    taches: problemes.map((p) => ({
      ...
      vehicule: infoVehicule(p.bon.vehicule),
...
async function chercherBonTravail(session, { numero, clientNom, statut, texte, vehicule } = {}) {
  if (!(await aAccesSection(session, "operations"))) {
    return { erreur: "Accès refusé à la section Opérations pour cet employé." };
  }
  if (!numero && !clientNom && !statut && !texte && !vehicule) return { erreur: "Précise un numéro de bon, un nom de client, un véhicule, un statut ou un texte." };
  ...
  if (vehicule) conditions.push(filtreMots(vehicule, [surVehicule]));
  if (texte) {
  ...
  const bons = await prisma.bonTravail.findMany({
    ...
    include: { client: true, vehicule: true, problemes: true, facture: true },
    orderBy: { creeLe: "desc" },
    take: 15,
  });
  ...
  return {
    bons: bons.map((b) => ({
      ...
      vehicule: infoVehicule(b.vehicule),
...
async function rendezVousDuJour(session, { date, dateFin, clientNom } = {}) {
  ...
  return {
    ...
    rendezVous: rendezVous.map((r) => ({
      ...
      adresse: [r.clientAdresse, r.clientVille, r.clientCodePostal].filter(Boolean).join(", ") || null,
      vehicule: r.vehiculeInfo || null,
      vehiculeDetails: r.vehiculeDetails ? infoVehicule(r.vehiculeDetails) : null,
      note: r.note || null,
      motif: r.motif,
      taches: r.taches?.length ? r.taches : undefined,
      statut: r.statut,
    })),
  };
}
...
// Dossier véhicule : retrouve un véhicule par plaque, NIV, année, marque,
// modèle ou version (ou par le nom du propriétaire) et retourne son
// propriétaire et l'historique de ses bons de travail.
async function accesVehicules(session) {
  return (await aAccesSection(session, "clients")) || (await aAccesSection(session, "operations"));
}

async function rechercherVehicule(session, { texte } = {}) {
  if (!(await accesVehicules(session))) {
    return { erreur: "Accès refusé aux sections Clients et Opérations pour cet employé." };
  }
  if (!texte || !String(texte).trim()) return { erreur: "Précise une plaque, un NIV ou un véhicule (ex: « Civic 2019 »)." };
  const voitBons = await aAccesSection(session, "operations");

  const vehicules = await prisma.vehicule.findMany({
    where: filtreMots(texte, [(mot) => ({ OR: conditionsVehicule(mot) }), contient("client.nom"), contient("client.telephone")]),
    include: {
      client: true,
      bons: voitBons ? { include: { problemes: true, facture: true }, orderBy: { creeLe: "desc" }, take: 10 } : false,
  ...
  return {
    vehicules: vehicules.map((v) => ({
      ...infoVehicule(v),
      nom: libelleVehicule(v) || v.plaque || "Véhicule",
      proprietaire: v.client.nom,
      telephoneProprietaire: v.client.telephone || null,
      lien: lienClient(v.client),
      nombreDeBons: v._count.bons,
      historiqueBons: voitBons
        ? v.bons.map((b) => ({
            numero: b.numero,
            lien: lienBon(b),
            statut: b.statut,
            date: b.datePrevue || b.creeLe,
            taches: b.problemes.map((p) => (p.notes ? `${p.description} — ${p.notes}` : p.description)),
            facture: b.facture ? { numero: b.facture.numero, statut: b.facture.statut, totalAvecTaxes: b.facture.totalAvecTaxes } : null,
          }))
        : undefined,
    })),
...
// Décode un NIV (année, marque, modèle, version) et indique s'il est déjà
// au dossier d'un client.
async function decoderNivVehicule(session, { niv } = {}) {
  if (!(await accesVehicules(session))) {
    return { erreur: "Accès refusé aux sections Clients et Opérations pour cet employé." };
  }
  const resultat = await decoderNiv(niv);
  if (resultat.erreur) return resultat;
  const auDossier = await prisma.vehicule.findMany({ where: { niv: resultat.niv }, include: { client: true }, take: 5 });
  return {
    ...resultat,
    source: resultat.source === "NHTSA" ? "Décodeur de la NHTSA" : "Hors ligne (année et marque seulement)",
    auDossier: auDossier.map((v) => ({ nom: libelleVehicule(v) || "Véhicule", proprietaire: v.client.nom, lien: lienClient(v.client) })),
  };
}

// Catalogue des listes déroulantes du dossier véhicule : marques offertes
// pour une année, modèles d'une marque, versions d'un modèle.
async function catalogueVehicules(session, { annee, marque, modele } = {}) {
  if (!(await accesVehicules(session))) {
    return { erreur: "Accès refusé aux sections Clients et Opérations pour cet employé." };
  }
  const an = annee ? Number(annee) : null;
  if (marque) {
    const cle = cleMarque(marque);
    if (!cle) return { erreur: `La marque « ${marque} » n'est pas dans le catalogue — elle peut quand même être entrée avec « Autre… ».` };
    if (modele) return { marque: nomMarque(cle), modele, versions: versionsDuModele(cle, modele) };
    return { marque: nomMarque(cle), annee: an, modeles: modelesPour(cle, an) };
  }
  const { populaires, autres } = marquesPour(an);
  return { annee: an, marquesCourantes: populaires.map((m) => m.nom), autresMarques: autres.map((m) => m.nom) };
}
...
async function rechercheGlobale(session, { texte } = {}) {
  ...
  const [resClients, resBons, resFactures, resSoumissions, resPieces, resFournisseurs, resRendezVous, resEmployes, resVehicules] = await Promise.all([
    clients ? rechercherClient(session, { nom: q }) : null,
    operations
      ? prisma.bonTravail.findMany({
          where: filtreMots(q, [
            contient("numero"), contient("client.nom"), surVehicule,
    ...
    clients || operations ? rechercherVehicule(session, { texte: q }) : null,
  ]);

  const resultat = { recherche: q };
  if (resClients?.clients?.length) resultat.clients = resClients.clients.slice(0, 8);
  if (resVehicules?.vehicules?.length) resultat.vehicules = resVehicules.vehicules.slice(0, 8);
  if (resBons?.length) resultat.bons = resBons.map((b) => ({ numero: b.numero, lien: lienBon(b), statut: b.statut, client: b.client.nom, vehicule: libelleVehicule(b.vehicule) || null, creeLe: b.creeLe }));
...
const OUTILS = [
  {
    ...
    declaration: {
      ...
      description: "Recherche un texte PARTOUT à la fois (clients, véhicules — plaque, NIV, marque, modèle —, bons et leurs tâches, factures, soumissions, pièces, fournisseurs, rendez-vous, employés). À utiliser quand la question est vague, quand on ne sait pas de quel type il s'agit (un nom, un numéro, un téléphone), ou quand une recherche ciblée n'a rien donné.",
      parameters: {
        type: "OBJECT",
  ...
  },
  {
    ...
    declaration: {
      ...
      description: "Recherche des clients par nom, téléphone, courriel, numéro de client, ville, ou par un de leurs véhicules (plaque, NIV, marque, modèle) — recherche partielle, mot par mot — et retourne leurs coordonnées et les véhicules de leur dossier.",
      parameters: {
        type: "OBJECT",
        properties: { nom: { type: "STRING", description: "Nom, téléphone, courriel, numéro, ville, plaque ou NIV du client (ou une partie)" } },
        required: ["nom"],
      },
    },
  },
  ...
  },
  {
    ...
    declaration: {
      ...
      description: "Recherche des bons de travail par numéro (partiel), nom ou téléphone du client, véhicule (plaque, NIV, année, marque, modèle), statut et/ou texte dans les tâches et leurs notes. Retourne statut, client, véhicule, tâches (avec notes), facture liée et date prévue.",
      parameters: {
        type: "OBJECT",
        properties: {
          ...
          vehicule: { type: "STRING", description: "Plaque, NIV, année, marque ou modèle du véhicule (ex: « ABC 123 », « Civic 2019 »)" },
        },
      },
    },
  },
  ...
  {
    permis: accesVehicules,
    executer: rechercherVehicule,
    declaration: {
      name: "rechercherVehicule",
      description: "Recherche dans les dossiers véhicules des clients par plaque, NIV, année, marque, modèle, version ou nom du propriétaire. Retourne le véhicule complet (année, marque, modèle, version, NIV, plaque), son propriétaire et l'historique des bons de travail faits dessus.",
      parameters: {
        type: "OBJECT",
        properties: { texte: { type: "STRING", description: "Plaque, NIV (ou une partie), ou description (ex: « Civic 2019 », « Ford F-150 Tremblay »)" } },
        required: ["texte"],
      },
    },
  },
  {
    permis: accesVehicules,
    executer: decoderNivVehicule,
    declaration: {
      name: "decoderNiv",
      description: "Décode un NIV (numéro d'identification du véhicule, 17 caractères) : valide le format et le chiffre de contrôle, donne l'année, la marque, le modèle et la version, et indique si ce véhicule est déjà au dossier d'un client.",
      parameters: {
        type: "OBJECT",
        properties: { niv: { type: "STRING", description: "Le NIV complet (17 caractères)" } },
        required: ["niv"],
      },
    },
  },
  {
    permis: accesVehicules,
    executer: catalogueVehicules,
    declaration: {
      name: "catalogueVehicules",
      description: "Consulte le catalogue de VR du logiciel (mêmes listes déroulantes que le dossier véhicule) : marques de VR, modèles d'une marque, ou types d'un modèle (Classe A, B, C, roulotte, sellette… — le ou les types connus du modèle en premier).",
      parameters: {
        type: "OBJECT",
        properties: {
          annee: { type: "NUMBER", description: "Année modèle (ex: 2019)" },
          marque: { type: "STRING", description: "Marque (ex: Honda)" },
          modele: { type: "STRING", description: "Modèle (ex: Civic) — avec la marque, retourne les versions" },
        },
      },
    },
  },
