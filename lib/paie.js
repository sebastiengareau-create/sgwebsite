...
async function calculerPaiePourEmploye(employeId, { periodeDebut, periodeFin, typePaie, heuresManuelles, montantVacances, boni } = {}) {
  ...
  if (typePaie === "VACANCES") {
    ...
    // Impôt basé sur les heures HABITUELLES (l'horaire par défaut), pas les
    // heures réelles de cette paie précise — sinon une semaine chargée (ou
    // creuse) fait varier les retenues d'une paie à l'autre pour rien. Si
    // l'horaire habituel est à 0 (aucun jour reconnu comme travaillé cette
    // période) on retombe sur les heures réelles, sinon l'impôt serait
    // calculé sur un salaire annuel de 0 $ alors que l'employé est payé.
    const heuresPourImpot = heuresParDefaut > 0 ? heuresParDefaut : heuresTravaillees;
    salaireAnnuelEstime = heuresPourImpot * taux * periodesParAn;
  }
